...
const buildUserResponse = async (user) => {
  ...
  if (!wallet) {
    wallet = { balances: { NGN: 0, GHS: 0, KES: 0 }, walletId: null, currency: 'NGN' };
  }
...
app.post('/api/bvn', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ success: true, user: await buildUserResponse(updatedUser) });
  } catch (e) {
...
app.listen(PORT, () => {
  console.log(`\n🚀 CreativePay API + Frontend running on port ${PORT}`);
