...
// ENV VALIDATION
['PAYSTACK_SECRET_KEY', 'JWT_SECRET', 'MONGODB_URI'].forEach(env => {
  if (!process.env[env]) { console.error(`❌ FATAL: ${env} missing`); process.exit(1); }
});
...
// SERVE STATIC FILES FROM ROOT - BULLETPROOF PATH
...
app.get('/api/partners/public', (req, res) => res.json({ success: true, data: [] }));
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!user ||!(await bcrypt.compare(password, user.password))) return res.status(401).json({ success: false, message: 'Invalid' });
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '7d' });
